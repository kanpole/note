...
```c
    当既需要原类提供的函数,又需要独立新增的函数时,设计一个适配器类,可以继承添加新方法,也可以包含添加新方法.使用者使用这些新方法就行了
...
```c
    builder类提供了一系列创建另一个对象的方法,用于设置属性，然后创建者,可以自己根据需要调用，进行构建，完成后再返回建造完成的对象
...
```c
    一个类,可以根据不同的状态属性,自己调整自己.每次调用状态函数,都会改变状态变量,然后根据不同的属性,状态会返回给使用者合适的状态对象,不需要外来干预，因为是状态自动生成相应的状态对象，所以需要提取指定有多少中状态
    场景：角色的行为树
```
...
```c
    通过建立一个内存池，也就是一个集合，这个集合存储各种对象，当需要某个对象时，就到池里查询，有就之间返回，没有就创建,并加入内存池.可以通过工厂方法模式来辅助实现
    场景：数据库链接池
