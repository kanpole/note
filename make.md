...
stdclib:stdclib(one.o) stdclib(two.o) #表示当one.o two.o更新时,重新构建此库
  ...
  $(MAKE) -C subdir                                    #-C表示进入subdir目录并执行make,当$(MAKE)作为规则命令，并且主make带有-n -t -q的时候，它也会执行这些参数
...
在模式规则中代表茎,也就是模式规则中%匹配的目标的部分,在非模式规则中,代表后辍之前的部分,如果是未识别的后辍,则为空.之所以去掉路径再匹配是因为要保证它们匹配之后在一个目录
$*
...
特殊变量
一般是不能在规则中使用自动变量的,但是可以使用这种方式来使用,也仅限这一个
...
```c
    ...
    -t #只更新目标文件的时间戳
    -w #切换目录时，打印进入的目录
    -W #假定目标需要更新,执行更新命令,可以和n配合使用查看依赖与此文件的目标
...
```
    ...
    target:;                   #空的不在隐含规则内的目标，总是被认为是最新的，以它为依赖的目标总是会被执行
