...
6 数组初始化可以={} 或者使用指定初始化{[2]=3,5},定义数组必须指明一个常量.int [] 这种不指明大小的形式,只能用在函数定义时,因为会被转换成指针
...
0 int fun(int num[static 20]) static新增的含义,用来限定参数至少有20个元素.为优化提供帮助.因为数组默认是用指针来传递,所以只能用这种方式来告诉编译器,数组的大小
